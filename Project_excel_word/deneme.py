...
DATABASE_DIR = os.path.join(os.path.expanduser('~/Desktop'), 'ExcelVeritabaniV3')

class ExcelYoneticiV2(tk.Tk):
    def __init__(self):
        super().__init__()
    ...
    def _arayuz_olustur(self):
        ...
        sol_frame.pack(side=tk.LEFT, fill=tk.Y, padx=10, pady=10)
        ttk.Label(sol_frame, text="Veritabanı", font=("Arial", 12, "bold")).pack(pady=(0,5))
        self.dosya_tree = ttk.Treeview(sol_frame, show="tree")
        self.dosya_tree.pack(fill=tk.Y, expand=True)
        self.dosya_tree.bind('<<TreeviewSelect>>', self._dosya_agacinda_secildi)
        # Sağ tık menüsü platforma göre bağlanıyor
        if sys.platform == 'darwin':
            self.dosya_tree.bind('<Button-2>', self._treeview_sag_tik_menu)  # Mac için sağ tık
            self.dosya_tree.bind('<Control-Button-1>', self._treeview_sag_tik_menu)  # Mac için ctrl+click
        self.dosya_tree.bind('<Button-3>', self._treeview_sag_tik_menu)  # Windows/Linux
        ttk.Button(sol_frame, text="Excel Yükle", command=self.dosya_yukle).pack(pady=10)

        # Orta panel: Filtre ve tablo
        orta_frame = ttk.Frame(ana_frame)
        orta_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=10, pady=10)
        self._olustur_filtre_paneli(orta_frame)
        self.tablo_frame = ttk.Frame(orta_frame)
    ...
    def _olustur_filtre_paneli(self, parent):
        self.filtre_frame = ttk.LabelFrame(parent, text="Filtreleme (Gelişmiş)")
        self.filtre_frame.pack(fill=tk.X, pady=5)
        self.filtre_sutun = ttk.Combobox(self.filtre_frame, state="disabled", width=20)
        self.filtre_sutun.grid(row=0, column=0, padx=5, pady=2)
        self.filtre_operator = ttk.Combobox(self.filtre_frame, state="disabled", width=15)
        self.filtre_operator.grid(row=0, column=1, padx=5, pady=2)
        self.filtre_giris1 = ttk.Entry(self.filtre_frame, width=15)
        self.filtre_giris1.grid(row=0, column=2, padx=5, pady=2)
        self.filtre_giris2 = ttk.Entry(self.filtre_frame, width=15)
        self.filtre_giris2.grid(row=0, column=3, padx=5, pady=2)
        self.filtre_giris2.grid_remove()
        self.filtre_btn = ttk.Button(self.filtre_frame, text="Filtre Ekle", command=self.filtre_ekle, state=tk.DISABLED)
        self.filtre_btn.grid(row=1, column=0, padx=5, pady=2, sticky="ew")
        self.filtre_temizle_btn = ttk.Button(self.filtre_frame, text="Tüm Filtreleri Temizle", command=self.filtre_temizle, state=tk.DISABLED)
        self.filtre_temizle_btn.grid(row=1, column=1, padx=5, pady=2, sticky="ew")
        # Word'e kaydet butonu
        self.kaydet_word_button = ttk.Button(self.filtre_frame, text="Word Olarak Kaydet", command=self.filtreli_veriyi_kaydet_word)
        self.kaydet_word_button.grid(row=1, column=2, padx=5, pady=2, sticky="ew")

        # Excel'e kaydet butonu
        self.kaydet_excel_button = ttk.Button(self.filtre_frame, text="Excel Olarak Kaydet", command=self.filtreli_veriyi_kaydet_excel)
        self.kaydet_excel_button.grid(row=1, column=3, padx=5, pady=2, sticky="ew")

        self.veri_duzenle_btn = ttk.Button(self.filtre_frame, text="Veri Düzenle", command=self.veri_duzenle_pencere, state=tk.NORMAL)
        self.veri_duzenle_btn.grid(row=1, column=4, padx=5, pady=2, sticky="ew")
        self.filtre_listbox = tk.Listbox(self.filtre_frame, width=80, height=3)
        self.filtre_listbox.grid(row=2, column=0, columnspan=4, padx=5, pady=2, sticky="ew")
        self.filtre_sil_btn = ttk.Button(self.filtre_frame, text="Seçili Filtreyi Kaldır", command=self.filtre_sil, state=tk.DISABLED)
        self.filtre_sil_btn.grid(row=3, column=0, padx=5, pady=2, sticky="ew")
        self.filtre_sonuc_label = ttk.Label(self.filtre_frame, text="")
        self.filtre_sonuc_label.grid(row=3, column=1, columnspan=3, sticky="w")

    def _filtre_paneli_guncelle(self):
        if self.df is not None:
            self.filtre_sutun['state'] = "readonly"
            self.filtre_sutun['values'] = list(self.df.columns)
            self.filtre_sutun.set("")
            self.filtre_operator['state'] = "readonly"
            self.filtre_operator.set("")
            self.filtre_giris1.delete(0, tk.END)
            self.filtre_giris2.delete(0, tk.END)
            self.filtre_btn['state'] = tk.NORMAL
            self.filtre_temizle_btn['state'] = tk.NORMAL
            self.filtre_sil_btn['state'] = tk.NORMAL
            self.filtre_sutun.bind('<<ComboboxSelected>>', self._filtre_sutun_degisti)
        else:
            self.filtre_sutun['state'] = "disabled"
            self.filtre_operator['state'] = "disabled"
            self.filtre_btn['state'] = tk.DISABLED
            self.filtre_temizle_btn['state'] = tk.DISABLED
            self.filtre_sil_btn['state'] = tk.DISABLED
    ...
    def _filtre_sutun_degisti(self, event):
        ...
        self.filtre_operator['values'] = ['Aralık', 'Anahtar Kelime']
        self.filtre_operator.set("")
        self.filtre_giris1.delete(0, tk.END)
        self.filtre_giris2.delete(0, tk.END)
        self.filtre_giris2.grid_remove()
    ...
    def veri_duzenle_pencere(self):
        ...
        # Filtre paneli
        filtre_frame = ttk.LabelFrame(pencere, text="Filtreleme (Geçici)")
        ...
        filtre_tur = ttk.Combobox(filtre_frame, state="readonly", width=15, values=["Aralık", "Anahtar Kelime"])
